...
describe("i18n route continuity", () => {
  ...
  it("redirects root path to detected language", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Sistemas e Estudos de Caso", level: 2 })).toBeInTheDocument();
  });
  ...
  it("redirects invalid language paths to /en fallback", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Selected Systems & Case Studies", level: 2 })).toBeInTheDocument();
  });
